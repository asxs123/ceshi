...
function Diy_continue() {
  ...
  chmod -R +x build 
  echo "DIY_P1_SH=build/common/diy-part1.sh" >> ${GITHUB_ENV}
  echo "DIY_P2_SH=build/common/diy-part2.sh" >> ${GITHUB_ENV}
  echo "CONFIGS=build/common/X86.config" >> ${GITHUB_ENV}
  echo "FEEDS_CONF=build/common/lean.feeds.conf.default" >> ${GITHUB_ENV}
}
...
function Diy_memu() {
  cd ${GITHUB_WORKSPACE}
  curl -fsSL https://raw.githubusercontent.com/asxs123/ceshi/123/common.sh -o common.sh
  if [[ $? -ne 0 ]]; then
    # curl下载失败，则执行wget下载。
    wget -q https://raw.githubusercontent.com/asxs123/ceshi/123/common.sh -O common.sh
  fi
  Diy_continue
}
